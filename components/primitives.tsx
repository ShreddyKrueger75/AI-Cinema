...
export function AssetImg({ src, ...rest }: React.ImgHTMLAttributes<HTMLImageElement> & { src: string | null | undefined }) {
  const resolved = useAssetUrl(src);
  if (!resolved) return null;
  return <img src={resolved} {...rest} />;
}

export function AssetVideo({ src, ...rest }: React.VideoHTMLAttributes<HTMLVideoElement> & { src: string | null | undefined }) {
  const resolved = useAssetUrl(src);
  if (!resolved) return null;
  return <video src={resolved} {...rest} />;
}

export function AssetAudio({ src, ...rest }: React.AudioHTMLAttributes<HTMLAudioElement> & { src: string | null | undefined }) {
  const resolved = useAssetUrl(src);
  if (!resolved) return null;
