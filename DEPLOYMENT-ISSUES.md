...
Create these GitHub issues before shipping to production:

---

## 🔴 BLOCKING: Set AUTH_SECRET in production

**Title:** Set AUTH_SECRET in production environment

**Body:**
BLOCKING: Required for deploy.

AUTH_SECRET must be set in Vercel environment variables before deploying to production. The app will throw on the first request without it.
...
**See:** app/auth.ts:9-11, app/login/page.tsx

**Labels:** security, blocking

---

## 🔴 BLOCKING: Set APP_URL for password reset emails

**Title:** Configure APP_URL for password reset email links

**Body:**
BLOCKING: Required for password reset flow to work.
...
## 🟡 HIGH: Verify provider environment variables

**Title:** Audit and set all provider API keys before deploy

**Body:**
Verify these are set in Vercel environment:
...
**Labels:** configuration, high

---

## 🟡 HIGH: Set up Vercel KV for user accounts

**Title:** Configure Vercel KV for production sign-in

**Body:**
User sign-in is currently disabled in production (shows "Cloud accounts are not configured").
...
## 📋 MEDIUM: Cache + refresh Replicate signed URLs

**Title:** Cache Replicate signed URLs, refresh on 403

**Body:**
Replicate motion URLs expire after 24 hours. Currently we re-fetch them from IDB at render time, but there's no refresh logic.
...
## 📋 MEDIUM: Preserve custom transitions across row reorder

**Title:** Custom transition settings lost when reordering sections

**Body:**
When you drag a section to reorder, custom transition settings (type, duration) reset to defaults.
...
## 📋 MEDIUM: LUT export not matching render output

**Title:** Color grading LUT export differs from ffmpeg render

**Body:**
When you export a .cube LUT file, the colors don't match what's rendered in the MP4.
...
## 🔵 LOW: File size limits (DoS prevention)

**Title:** Add file size caps on imports

**Body:**
Prevent abuse by capping:
...
**Title:** Post-deploy monitoring checklist

**Body:**
After shipping, monitor:

- [ ] sessionStorage draft recovery working (check browser console for errors)
- [ ] Delete confirmations rendering (test in production UI)
- [ ] Auth errors user-friendly (test /login with wrong password)
- [ ] Render jobs complete without orphans (check for stray processes)
- [ ] Music/graphics rendering correctly in MP4s (spot-check exports)
